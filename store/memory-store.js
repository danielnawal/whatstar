...
class ConcurrentStore extends EventEmitter {
    constructor(options = {}) {
        ...
        this.config = {
            ...
            // **Chat pruning — keeps store file from growing indefinitely**
            maxChats: options.maxChats || 0,              // 0 = no hard cap
            chatInactiveDays: options.chatInactiveDays || 0, // 0 = no pruning by age
            ...options
        };
    ...
    async serializeStoreData() {
        // --- Chat pruning: remove inactive chats before writing to disk ---
        const inactiveDays = this.config.chatInactiveDays;
        const maxChats    = this.config.maxChats;
        const cutoffMs    = inactiveDays > 0 ? (Date.now() / 1000) - (inactiveDays * 86400) : 0;

        let chatEntries = [...this.chats.entries()];

        // Remove chats older than chatInactiveDays (uses conversationTimestamp in seconds)
        if (cutoffMs > 0) {
            chatEntries = chatEntries.filter(([, chat]) => {
                const ts = chat?.conversationTimestamp ?? 0;
                return ts === 0 || ts >= cutoffMs;
            });
        }

        // Keep only the most recent maxChats if limit set
        if (maxChats > 0 && chatEntries.length > maxChats) {
            chatEntries.sort((a, b) => (b[1]?.conversationTimestamp ?? 0) - (a[1]?.conversationTimestamp ?? 0));
            chatEntries = chatEntries.slice(0, maxChats);
        }
