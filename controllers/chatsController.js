...
/**
 * POST /chats/send-list?id=SESSION
 * body: { receiver, title, body, footer, buttonText, sections }
...
const sendList = async (req, res) => {
    ...
    try {
        const exists = await isExists(session, receiver)
        if (!exists) return response(res, 400, false, 'The receiver number does not exist.')

        await sendListMessage(session, receiver, {
        ...
        response(res, 200, true, 'List message sent successfully.')
    } catch {
        response(res, 500, false, 'Failed to send list message.')
    }
}

/**
 * POST /chats/send-buttons?id=SESSION
 * body: { receiver, text, footer, buttons:[{ id, text }] }  (máx 3 botones)
 */
const sendButtons = async (req, res) => {
    ...
    try {
        ...
        await sendButtonMessage(session, receiver, {
            text:    req.body.text    ?? '',
            footer:  req.body.footer  ?? '',
            buttons: req.body.buttons ?? [],
        })
        response(res, 200, true, 'Button message sent successfully.')
    } catch {
        response(res, 500, false, 'Failed to send button message.')
    }
}

export { getList, send, sendBulk, deleteChat, read, forward, sendPresence, downloadMedia, sendList, sendButtons }
