...
trait Whatsapp
{
    private function messageSend($data,$from, $reciver,$type,$filter=false,$delay = 0,$instant = false)
    {
        if (!$instant) {
    ...
    private function formatArray($data,$message,$type)
    {
        ...
        elseif ($type == 'text-with-media') {
            ...
            if (in_array($file_type, ['pdf','docx','xlsx','csv','txt'])) {
                $fileName = !empty($data['attachment_name']) ? $data['attachment_name'] : basename($data['attachment']);
