...
class CronController extends Controller
{
    ...
    {
       $willExpireFrom = today()->addDays(1)->format('Y-m-d');
       $willExpireTo   = today()->addDays(7)->format('Y-m-d');
       $users = User::whereHas('subscription', function($q) {
                    $q->where('status', 1)->where('price', '>', 0);
               })->with('subscription')->whereBetween('will_expire',[$willExpireFrom,$willExpireTo])->latest()->get();

       foreach ($users as $key => $user) {
           $this->sentWillExpireWhatsapp($user);
       }
    ...
    /**
     * Envía aviso de vencimiento por WhatsApp al dispositivo del usuario
     */
    private function sentWillExpireWhatsapp($user)
    {
        // Notificación interna en el panel
        $willExpire = Carbon::parse($user->will_expire)->format('d/m/Y');
        $planName   = $user->subscription->title ?? 'tu plan';
        $planId     = $user->plan_id;

        $notification['user_id'] = $user->id;
        $notification['title']   = 'Aviso de vencimiento de suscripción';
        $notification['comment'] = 'Tu suscripción vence el ' . $willExpire;
        $notification['url']     = '/user/subscription/' . $planId;
        $this->createNotification($notification);

        // Obtener el número WhatsApp del cliente (primer dispositivo activo)
        $userDevice = Device::where('user_id', $user->id)
                            ->where('status', 1)
                            ->whereNotNull('phone')
                            ->first();

        if (empty($userDevice) || empty($userDevice->phone)) {
            return;
        }

        $senderDeviceId = env('NOTIFICATION_DEVICE_ID', 44);
        $phone          = preg_replace('/[^0-9]/', '', $userDevice->phone);

        $message  = "Hola *{$user->name}*, le informamos que su suscripción al plan *{$planName}* vence el *{$willExpire}*.\n\n";
        $message .= "Para renovarla ingrese al siguiente enlace:\n";
        $message .= env('APP_URL') . "/user/subscription/{$planId}\n\n";
        $message .= "Si ya realizó su pago, por favor ignore este mensaje.";
