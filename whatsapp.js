...
// Forzar siempre HTTPS para evitar 301 redirect que convierte POST→GET
const APP_URL = (process.env.APP_URL ?? '').replace(/^http:\/\//, 'https://')
...
const createSession = async (sessionId, res = null, options = { usePairingCode: false, phoneNumber: '' }) => {
    ...
    const store = makeInMemoryStore({
        ...
        maxChats: 1500,          // nunca más de 1500 chats por dispositivo en disco
        chatInactiveDays: 7,     // purgar chats sin actividad en más de 7 días
    ...
    wa.ev.on('messages.upsert', async (m) => {
        console.log(`[${sessionId}] messages.upsert recibido: ${m.messages.length} mensajes, tipo: ${m.type}`)
        ...
        if (messages.length > 0) {
            ...
            // Enviar mensajes al webhook de Laravel para auto-respuesta
            console.log(`[${sessionId}] Procesados ${messageTmp.length} mensajes, enviando webhook a ${APP_URL}`)
            if (messageTmp.length > 0 && APP_URL) {
                const data = messageTmp.map(msg => {
                    const enriched = {
                        ...msg,
                        key: msg.key ? { ...msg.key, remoteJidAlt: msg.key.remoteJid } : msg.key
                    }

                    // ── Extraer texto de respuestas interactivas ──────────────
                    if (msg.message) {
                        // Respuesta a lista
                        if (msg.message.listResponseMessage) {
                            const lr = msg.message.listResponseMessage
                            enriched._interactive = {
                                type: 'list_response',
                                selectedId: lr.singleSelectReply?.selectedRowId ?? '',
                                selectedText: lr.title ?? ''
                            }
                        }
                        // Respuesta a botones
                            ...
                            enriched._interactive = {
                                ...
                                selectedText: br.selectedDisplayText ?? ''
                            }
                        }
                        // Respuesta a template button
                        else if (msg.message.templateButtonReplyMessage) {
                            const tr = msg.message.templateButtonReplyMessage
                            enriched._interactive = {
                                type: 'template_response',
                                selectedId: tr.selectedId ?? '',
                                selectedText: tr.selectedDisplayText ?? ''
                            }
                        }
                    }
                ...
                axios.post(`${APP_URL}/api/send-webhook/${sessionId}`, {
                    type: 'MESSAGES_UPSERT',
                    data
                }).then(r => console.log(`[${sessionId}] Webhook OK: ${r.status}`))
                  .catch(e => console.log(`[${sessionId}] Webhook ERROR: ${e.message}`))
            }
        }
    })
    ...
    wa.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'open') {
            ...
            try {
                await axios.post(`${APP_URL}/api/set-device-status/${sessionId}/1`)
            } catch {}
        }
...
const isExists = async (session, jid, isGroup = false) => {
    try {
        ...
        // LID addresses are valid WhatsApp identifiers, skip the check
        if (jid.endsWith('@lid')) {
            return true
        }
...
/**
 * Envía un mensaje de LISTA interactiva (hasta 10 opciones en secciones).
 * @param {object} params
 * @param {string} params.title       - Título del mensaje
 * @param {string} params.body        - Cuerpo / descripción
 * @param {string} params.footer      - Pie (opcional)
 * @param {string} params.buttonText  - Texto del botón que abre la lista
 * @param {Array}  params.sections    - [{title, rows:[{rowId, title, description}]}]
 */
const sendListMessage = async (session, receiver, params, delayMs = 500) => {
    try {
        await delay(parseInt(delayMs))
        return await session.sendMessage(receiver, {
            listMessage: {
                ...
                buttonText: params.buttonText || 'Ver opciones',
                listType: 1,
 ...
 * @param {string} params.footer  - Pie (opcional)
 * @param {Array}  params.buttons - [{id, text}] máx 3
 */
const sendButtonMessage = async (session, receiver, params, delayMs = 500) => {
    try {
        await delay(parseInt(delayMs))
        const buttons = (params.buttons || []).slice(0, 3).map((b, i) => ({
...
const getStoreMessage = async (session, messageId, remoteJid) => {
    try {
        // loadMessages returns an array when called with a messageId.
        // Try the original JID first, then the s.whatsapp.net variant as fallback
        // (store may normalize @lid differently depending on Baileys version).
        const jidsToTry = [remoteJid]
        if (remoteJid.includes('@lid')) {
            jidsToTry.push(remoteJid.replace('@lid', '@s.whatsapp.net'))
        } else if (remoteJid.includes('@s.whatsapp.net')) {
            jidsToTry.push(remoteJid.replace('@s.whatsapp.net', '@lid'))
        }

        for (const jid of jidsToTry) {
            const result = await session.store.loadMessages(jid, messageId)
