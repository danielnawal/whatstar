      ...
      <div class="row g-0 align-items-end align-items-lg-center">
         ...
         <div class="col-xl-4 col-lg-4 col-md-6 mb-30 wow tpfadeLeft" data-wow-duration=".9s" data-wow-delay=".5s">
            <div class="tp-price__item {{ $plan->is_recommended == 1 ? 'tp-price__active z-index' : '' }}">
               ...
               <h3 class="tp-price__title">{{ amount_format($plan->price,'icon') }} <small class="tp-price__small_title">{{ __($plan->days == 30 ? '/month' : '/year') }}</small></h3>
               <div class="tp-price__list">
                  <ul>
                     ...
                     <li class="{{ planData($key,$data)['value'] == false && planData($key,$data)['is_bool'] == true ? 'd-none' : '' }}">
                        @php
                           $rawTitle = planData($key,$data)['title'];
                           $parenPos = strpos($rawTitle, ' (');
                           if ($parenPos !== false) {
                               $labelKey = ucfirst(str_replace('_', ' ', substr($rawTitle, 0, $parenPos)));
                               $displayTitle = __($labelKey) . substr($rawTitle, $parenPos);
                           } else {
                               $displayTitle = __(ucfirst(str_replace('_', ' ', $rawTitle)));
                           }
                        @endphp
                        {{ $displayTitle }}
                     </li>
